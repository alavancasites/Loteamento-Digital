...
class AvisoController extends GxController {

	
    public function getRepresentingFields(){
		return Aviso::representingColumn();
	}
    
	public function actionView($id) {
		$this->render('view', array(
			'model' => $this->loadModel($id, 'Aviso'),
		));
	}
	...
	public function actionCreate() {
		...
		if (isset($_POST['Aviso'])) {
			$model->setAttributes($_POST['Aviso']);

			if ($model->save()) {
				if (Yii::app()->getRequest()->getIsAjaxRequest())
					Yii::app()->end();
				else
					$this->redirect($this->createUrlRel('view',array('id' => $model->idaviso,'success'=>'create')));
			}
		}
        else{
	...
	public function actionUpdate($id) {
		...
		if (isset($_POST['Aviso'])) {
			...
			if ($model->save()) {
                $this->redirect($this->createUrlRel('view',array('id' => $model->idaviso,'success'=>'update')));
			}
		}
