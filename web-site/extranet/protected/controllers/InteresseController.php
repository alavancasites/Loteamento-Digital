...
class InteresseController extends GxController {

	
    public function getRepresentingFields(){
		return Interesse::representingColumn();
	}
    
	public function actionView($id) {
		$this->render('view', array(
			'model' => $this->loadModel($id, 'Interesse'),
		));
	}
	...
	public function actionCreate() {
		...
		if (isset($_POST['Interesse'])) {
			$model->setAttributes($_POST['Interesse']);

			if ($model->save()) {
				if (Yii::app()->getRequest()->getIsAjaxRequest())
					Yii::app()->end();
				else
					$this->redirect($this->createUrlRel('view',array('id' => $model->idinteresse,'success'=>'create')));
			}
		}
        else{
	...
	public function actionUpdate($id) {
		...
		if (isset($_POST['Interesse'])) {
			...
			if ($model->save()) {
                $this->redirect($this->createUrlRel('view',array('id' => $model->idinteresse,'success'=>'update')));
			}
		}
	...
	{
		$model=Interesse::model()->findByPk($id);
		if($model===null)
	...
	public function actionIndex() {
		...
		if(isset($_GET['o']) && isset($_GET['f']) ){
        	...
        	$criteria->order = 'idinteresse desc';
        }
		...
		$dataProvider = new CActiveDataProvider('Interesse', array(
            'criteria'=>$criteria,
			'pagination' => array(
