...
class DestaqueController extends GxController {

	
    public function getRepresentingFields(){
		return Destaque::representingColumn();
	}
    
	public function actionView($id) {
		$this->render('view', array(
			'model' => $this->loadModel($id, 'Destaque'),
		));
	}
	...
	public function actionCreate() {
		...
		if (isset($_POST['Destaque'])) {
			$model->setAttributes($_POST['Destaque']);

			if ($model->save()) {
				if (Yii::app()->getRequest()->getIsAjaxRequest())
					Yii::app()->end();
				else
					$this->redirect($this->createUrlRel('view',array('id' => $model->iddestaque,'success'=>'create')));
			}
		}
        else{
	...
	public function actionUpdate($id) {
		...
		if (isset($_POST['Destaque'])) {
			...
			if ($model->save()) {
                $this->redirect($this->createUrlRel('view',array('id' => $model->iddestaque,'success'=>'update')));
			}
		}
	...
	{
		$model=Destaque::model()->findByPk($id);
		if($model===null)
