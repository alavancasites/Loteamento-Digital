...
class NewsletterController extends GxController {
    ...
    public function getRepresentingFields(){
		return Newsletter::representingColumn();
	}
		...
		if (isset($_POST['Newsletter'])) {
			...
			if ($model->save()) {
				if (Yii::app()->getRequest()->getIsAjaxRequest())
					...
					$this->redirect($this->createUrlRel('view',array('id' => $model->idnewsletter,'success'=>'create')));
			}
		}
        else{
	...
	public function actionUpdate($id) {
		...
		if (isset($_POST['Newsletter'])) {
			...
			if ($model->save()) {
                $this->redirect($this->createUrlRel('view',array('id' => $model->idnewsletter,'success'=>'update')));
			}
		}
	...
	public function actionIndex() {
		...
		if(isset($_GET['o']) && isset($_GET['f']) ){
        	...
        	$criteria->order = 'idnewsletter desc';
        }
