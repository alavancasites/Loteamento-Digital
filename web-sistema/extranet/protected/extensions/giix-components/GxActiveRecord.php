...
abstract class GxActiveRecord extends CActiveRecord {
    ...
    */
    ...
    public function find($condition,$params=array()){
        $condition = $this->tratarCriteria($condition);
    ...
    public function setAtributosPadroes(){
        if(get_class($this)=='Cliente'){
            return;
        }
        if(get_class($this)=='CorretorCliente'){
            return;
        }
        if(get_class($this)=='UserCliente'){
            return;
        }

        if($_SESSION['filtro_cliente']){
            $attributes = array_keys($this->attributeLabels());
            if(in_array('idcliente',$attributes) ){
                $this->idcliente = $_SESSION['filtro_cliente'];
            }
        }
        ...
        if(Yii::app()->user->obj->idUserGroup==1){
            return;
        }
        ...
        $attributes = array_keys($this->attributeLabels());
        if(in_array('idcliente',$attributes) ){
            if(Yii::app()->user->filtro_cliente){
                $this->idcliente = Yii::app()->user->filtro_cliente;
            }
        }
    }
    ...
    public function tratarCriteria($criteria){
        ...
        if($_SESSION['filtro_cliente']){
            if(is_object($criteria)){
                if(isset($relations['cliente']) || in_array('idcliente',$attributes) ){
                    $criteria->addCondition("t.idcliente = '".$_SESSION['filtro_cliente']."'");
                }
                // if(isset($relations['clientes'])){
                //     if(is_array($criteria->with)){
                //         $criteria->with[] = 'clientes';
                //     }else{
                //         $criteria->with = array('clientes');
                //     }
                //     $criteria->together = true;
                //     $criteria->addCondition("clientes.idcliente = '".$_SESSION['filtro_cliente']."'");
                //     echo 'id'.$_SESSION['filtro_cliente'].'--';
                // }
            }
        }
        ...
        if(is_object($criteria)){
            if(isset($relations['cliente']) || in_array('idcliente',$attributes) ){
                $criteria->addCondition("t.idcliente = '".Yii::app()->user->filtro_cliente."'");
            }
            // print_r($relations);
            // exit;
            if(isset($relations['clientes'])){
                ...
                $criteria->together = true;
                $criteria->addCondition("clientes.idcliente = '".Yii::app()->user->filtro_cliente."'");
                // echo 'id'.Yii::app()->user->filtro_cliente.'--';
            }
        }
        // print_r($criteria);


        // $estabelecimento_tabela = Estabelecimento::model()->tableName();
        // if(in_array('idestabelecimento',$attributes) && $this->tableName() != 'estabelecimento' ){
        // 	$criteria->join .= ' inner join '.$estabelecimento_tabela.' as estabelecimento_active_record on estabelecimento_active_record.idestabelecimento = t.idestabelecimento ';
        // 	$criteria->addCondition("estabelecimento_active_record.idcliente = '".Yii::app()->user->obj->idcliente."'");
        // }
        //
        // if((is_array($criteria->with) && in_array('estabelecimentos',$criteria->with))){
        // 	$criteria->addCondition("estabelecimentos.idcliente = '".Yii::app()->user->obj->idcliente."'");
        // }
