...
class SiteController extends Controller {
    ...
    public function actionGetBairros(){
        $cidades = Bairro::model()->findAll(array(
            'condition' => "idcidade = '".$_GET['idcidade']."'",
            'order' => 'nome',
        ));
        $cidades_list = CHtml::listData($cidades,'idbairro','nome');
        echo CJSON::encode($cidades_list);
        Yii::app()->end();

    }

    public function actionCreateBairro() {
        $model = new Bairro;

        if (isset($_POST['Bairro'])) {
            Utf8::decode($_POST);
            $model->setAttributes($_POST['Bairro']);

            if ($model->save()) {
                echo CJSON::encode(array('success'=>true, 'idcidade'=>$model->idcidade));
                exit;
            }
        }

        echo CJSON::encode(array('success'=>false, 'erro'=>CHtml::errorSummary($model)));
        exit;
    }
    ...
    */
    public function actionIndex() {
        if (Yii::app()->user->isGuest) {
            ...
            if(Yii::app()->user->obj->idUserGroup!='1'&&Yii::app()->user->filtro_cliente==0 ){
                $this->render('selecao_cliente');
            }else{
                $this->render('index');
            }
    ...
    public function actionCliente() {
        ...
        if($_GET['cliente']!=''){
            ...
            if(is_object($model)){
                try {
                    Yii::app()->user->filtro_cliente = $model->idcliente;
                ...
                // echo Yii::app()->user->filtro_cliente; exit;
                $this->redirect("index");
                exit;
            }
        }
